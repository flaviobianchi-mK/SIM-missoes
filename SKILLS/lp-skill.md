...
Quando esta skill gerar output HTML/CSS (além de copy), aplicar obrigatoriamente o design system SIM × Apple definido em `CLAUDE.md` e `skill-pro-uidesigner.md`.

### Tipografia

- **Display / Hero:** `'Lexend Deca', sans-serif` — `--text-display` (3.8125rem) · peso 700 · `letter-spacing: -0.71px`
- **Body / CTA:** `'Comfortaa', sans-serif` — `--text-base` (1rem) · peso 400
...
### Estrutura de seções — Apple

| Seção de Copy | Componente Apple |
|---|---|
| Hero | Hero Section (`specs-apple-design.md §4`) |
| Problema | Story Section scroll narrativo (`§8`) |
| Benefícios | Feature / Icon Cards (`§9`) |
| Prova / Números | Stat Cards (`§10`) |
| Oferta | Bento Gallery ou Feature Cards (`§7/§9`) |
| CTA Final | CTA Section com botão `--radius-pill` |
...
--color-accent: var(--color-accent);      /* CTA — brand guide SIM */
--radius-pill:  50px;                     /* botão CTA principal */
--section-padding-v: 100px;              /* desktop */
--max-content-width: 980px;
--shadow-card: 0 2px 24px rgba(0,0,0,0.12);
```

### Regras de output HTML

- Cada seção de copy mapeia para um `<section>` semântico
- Nav sticky com `backdrop-filter: saturate(180%) blur(20px)`
- Breakpoints Apple: `max-width: 734px` (mobile) · `min-width: 1069px` (desktop)
